...
const Notestate=(props)=>{
  ...
  const host="http://localhost:5000"
    ...
    const [res,setres]=useState([false,"value",false])

    //get all notes
    const storeval=async(n,p,k,t,h,ph,rain)=>{
      const response=await fetch(`${host}/api/crop`,{
        method:'post',
        headers: {
          'Content-Type': 'application/json',
        }, 
        body: JSON.stringify({n,p,k,t,h,ph,rain}) 
      })
      const json=await response.json()
      console.log(json.output)
      setres([true,json.output,true])
    ...
    const addnote=async(name,list,location,number)=>{
      //To do API call
      const response = await fetch(`${host}/api/notes/Addnotes`, {
        ...
        body: JSON.stringify({name,list,location,number}) 
      });
    ...
    return(
        <noteContext.Provider value={{notes,res,getnotes,addnote,deletenote,editnote,storeval,lang,langtoggle}}>
            {props.children}
